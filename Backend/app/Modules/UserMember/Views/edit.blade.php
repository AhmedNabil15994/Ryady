...
@section('title','اعضاء مجتمع الشاب الريادي - تعديل')
        ...
        <div class="d-flex align-items-center">
            ...
            <div class="main-menu dropdown dropdown-inline">
                ...
                <div class="dropdown-menu" dropdown-toggle="hover">
                    @if(\Helper::checkRules('add-user-member'))
                    <a href="{{ URL::to('/userMembers/add') }}" class="dropdown-item">
                        <i class="m-nav__link-icon fa fa-plus"></i>
                        <span class="m-nav__link-text">اضافة</span>
                    </a>
                    @endif
                    @if(\Helper::checkRules('sort-user-member'))
                    <a href="{{ URL::to('/userMembers/arrange') }}" class="dropdown-item">
                        <i class="m-nav__link-icon fa fa-sort-numeric-up"></i>
                        <span class="m-nav__link-text">ترتيب</span>
                    </a>
                    @endif
                    @if(\Helper::checkRules('charts-user-member'))
                    <a href="{{ URL::to('/userMembers/charts') }}" class="dropdown-item">
    ...
    <div class="card-body">
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="AddTabs" role="tabpanel">
                <form class="forms m-form m-form--group-seperator-dashed" method="POST" action="{{ URL::to('/userMembers/update/'.$data->data->id) }}">  
                    @csrf
                     ...
                     <div class="form-group m-form__group row" style="padding-right: 0;padding-left: 0;padding-bottom: 10px;">
                        <div class="col-lg-12">
                            ...
                            <select name="shown" class="form-control select2">
                                <option value="0" {{ $data->data->shown == 0 ? 'selected' : '' }}>لا</option>
                                <option value="1" {{ $data->data->shown == 1 ? 'selected' : '' }}>نعم</option>
                            </select>   
                        </div>
                    </div>
                </form>

...
@section('title','القوائم السفلية - تعديل')
        ...
        <div class="d-flex align-items-center flex-wrap mr-1">
            ...
            <div class="d-flex align-items-baseline flex-wrap mr-5">
                ...
                <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                    ...
                    <li class="breadcrumb-item">
                        <a href="{{ URL::to('/bottomMenu') }}" class="text-muted">القوائم السفلية</a>
                    </li>
                    <li class="breadcrumb-item">
        ...
        <div class="d-flex align-items-center">
            ...
            <div class="main-menu dropdown dropdown-inline">
                ...
                <div class="dropdown-menu" dropdown-toggle="hover">
                    @if(\Helper::checkRules('add-bottomMenu'))
                    <a href="{{ URL::to('/bottomMenu/add') }}" class="dropdown-item">
                        <i class="m-nav__link-icon fa fa-plus"></i>
                        <span class="m-nav__link-text">اضافة</span>
                    </a>
                    @endif
                    @if(\Helper::checkRules('sort-bottomMenu'))
                    <a href="{{ URL::to('/bottomMenu/arrange') }}" class="dropdown-item">
                        <i class="m-nav__link-icon fa fa-sort-numeric-up"></i>
                        <span class="m-nav__link-text">ترتيب</span>
                    </a>
                    @endif
                    @if(\Helper::checkRules('charts-bottomMenu'))
                    <a href="{{ URL::to('/bottomMenu/charts') }}" class="dropdown-item">
    ...
    <div class="card-body">
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="AddTabs" role="tabpanel">
                <form class="forms m-form m-form--group-seperator-dashed" method="POST" action="{{ URL::to('/bottomMenu/update/'.$data->data->id) }}">  
                    @csrf
                    ...
                    <div class="form-group m-form__group row" style="padding-right: 0;padding-left: 0;padding-bottom: 10px;">
                        <div class="col-lg-12">
                            ...
                            <select name="page_id" class="form-control select2" id="kt_select2_1">
                                <option value="0" {{ $data->data->page_id == 0 ? 'selected' : '' }}>صفحة خارجية</option>
                                @foreach($data->pages as $page)
                                <option value="{{ $page->id }}" {{ $data->data->page_id == $page->id ? 'selected' : '' }}>{{ $page->title }}</option>
                                @endforeach
                            </select>
                            <span class="m-form__help LastUpdate">تم الحفظ فى :  {{ $data->data->created_at }}</span>
                        </div>
                    ...
                    <div class="form-group m-form__group row" style="padding-right: 0;padding-left: 0;padding-bottom: 10px;">
                        <div class="col-lg-12">
                            ...
                            <input class="form-control m-input" type="text" name="link" value="{{ $data->data->link }}" maxlength="" placeholder="">
                            <span class="m-form__help LastUpdate">تم الحفظ فى :  {{ $data->data->created_at }}</span>
                        </div>
                    </div>
                    <div class="form-group m-form__group row" style="padding-right: 0;padding-left: 0;padding-bottom: 10px;">

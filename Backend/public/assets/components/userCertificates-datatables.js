		...
		var DataTable = table.DataTable({
			...
			ajax: {
				...
				data:function(dtParms){
			       	dtParms.start_date = $('#kt_datetimepicker_7_1').val();
			       	dtParms.end_date = $('#kt_datetimepicker_7_3').val();
			       	dtParms.membership_id = $('select[name="membership_id"]').val();
			        dtParms.columnsDef= [
						'id', 'username','code','membership_name','start_date','end_date'];
			        return dtParms
			    }
			},
			columns: [
					...
					render: function(data, type, full, meta) {
						...
						return '<div class="main-menu dropdown dropdown-inline">'+
		                            ...
		                            '<div class="dropdown-menu" dropdown-toggle="hover">'+
		                                '<a href="'+myURL+'/download/'+full.id+'" class="dropdown-item">'+
		                                    '<i class="m-nav__link-icon fa fa-download"></i>'+
		                                    '<span class="m-nav__link-text">تحميل</span>'+
		                                '</a>'+
		                            '</div>'+
		                        '</div>';
					},
